...
function shouldUpdate(
  ...
  // Prevent slug generation if it's supposed to be empty (e.g., for the homepage)
  if (computedField === 'slug' && val.is_homepage && !val[computedField]) {
    return false; // Prevent generating the slug if it should remain empty
  }
  ...
  if (!changedFields.length) {
    // Do not update if no fields have changed
    return false;
  }
...
export const useDeepValues = (
  ...
  watch(
    ...
    async (val, oldVal) => {
      ...
      // Prevent slug from being generated if it should remain empty
      if (valObj.is_homepage && !valObj.slug) {
        return;
      }
