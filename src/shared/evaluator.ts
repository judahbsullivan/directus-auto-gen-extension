    ...
    if (values) {
        ...
        if (opMatch) {
            ...
            if (args.length === 1) {
                ...
                if (op === 'ASUM') {
                    // aggregated sum
                    return (values[args[0]] as unknown[])?.reduce((acc: any, item) => acc + parseExpression(args[1], item as typeof values, {}, debug), 0) ?? 0;
                }
                if (op === 'AMIN') {
                    // aggregated min
                    return (values[args[0]] as unknown[])?.reduce((acc: any, item) => Math.min(acc, parseExpression(args[1], item as typeof values, {}, debug)), Infinity) ?? 0;
                }
                if (op === 'AMAX') {
                    // aggregated max
                    return (values[args[0]] as unknown[])?.reduce((acc: any, item) => Math.max(acc, parseExpression(args[1], item as typeof values, {}, debug)), -Infinity) ?? 0;
                }
                if (op === 'AAVG') {
                    // aggregated average
                    const arr = (values[args[0]] as any[]) ?? [];
                    return arr.reduce((acc: any, item: any) => acc + parseExpression(args[1], item as typeof values, {}, debug), 0) / arr.length;
                }
                if (op === 'AMUL') {
                    // aggregated multiplication
                    return (values[args[0]] as unknown[])?.reduce((acc: any, item) => acc * parseExpression(args[1], item as typeof values, {}, debug), 1) ?? 0;
                }
                if (op === 'AAND') {
                    // aggregated and
                    return (values[args[0]] as unknown[])?.reduce((acc: any, item) => acc && parseExpression(args[1], item as typeof values, {}, debug), true) ?? false;
                }
                if (op === 'AOR') {
                    // aggregated or
                    return (values[args[0]] as unknown[])?.reduce((acc: any, item) => acc || parseExpression(args[1], item as typeof values, {}, debug), false) ?? false;
                }
                if (op === 'ACOUNT') {
                    // aggregated count
                    return (values[args[0]] as unknown[])?.reduce((acc: any, item) => acc + (parseExpression(args[1], item as typeof values, {}, debug) ? 1 : 0), 0) ?? 0;
                }
                // loop operators
                if (op === 'MAP') {
                    // map
...
export const toSlug = (value: unknown): string => {
    // 1. Safety Check: specific handling for null/undefined
    if (value === null || value === undefined) return '';

    // 2. Convert to string (handles numbers, booleans)
